...
def helix(r0, r_eps, p, h, depth=0, frac=0.025): #1e-1):
    def func(t):
        ...
        print(f"helix.func:+ {helixCount}: t={t:.4f} r={r0:.3f} t={r_eps:.3f} p={p:.3f} h={h:.3f} i={depth:.3f} f={frac:.3f}")

        fa: float = 0

        if t > frac and t < 1 - frac:
            fa = pi / 2
            print(f"helix.func:  {helixCount}: t={t:.4f} fa={degrees(fa):.3f} MIDDL {t:.4f} > {frac:.3f} and {t:.4f} < {(1 - frac):.3f}")
            z = h * t + depth
            r = r0 + r_eps
        elif t <= frac:
            fa = (pi / 2 * t / frac)
            ...
            print(f"helix.func:  {helixCount}: fa={degrees(fa):.3f} FIRST {t:.4f} <= {frac:.3f} ")
        else:
            fa = (2 * pi - pi / 2 * (1 - t) / frac)
            print(f"helix.func:  {helixCount}: t={t:.4f} fa={degrees(fa):.3f} LAST  {t:.4f} >= {frac:.3f} ")
            z = h * t - depth * sin(fa) #2 * pi - pi / 2 * (1 - t) / frac)
            r = r0 - r_eps * sin(fa) #2 * pi - pi / 2 * (1 - t) / frac)

        print(f"helix.func:  r={r:.3f} radius={r0:.3f} + (threadDeptht={r_eps} * fadeScale={sin(fa)})")

        a = 2 * pi / (p / h) * t
        x = r * sin(-a) #-2 * pi / (p / h) * t)
        y = r * cos(a) #2 * pi / (p / h) * t)

        print(f"helix.func:- {helixCount}: t={t:.4f} r={r:.3f} a={degrees(a):.3f} fa={degrees(fa):.3f} fs={sin(fa):.3f} ({x:.3f}, {y:.3f}, {z:.3f})")
        helixCount += 1
        return x, y, z
...
def thread(radius, pitch, height, depth, radius_eps, aspect=10):
    ...
    e1_top = (
        cq.Workplane("XY").parametricCurve(helix(radius, 0, pitch, height, depth, 1 / aspect)).val()
    )
    show(e1_top, "e1_top")
